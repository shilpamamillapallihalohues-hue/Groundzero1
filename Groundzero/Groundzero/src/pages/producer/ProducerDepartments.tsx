...
export default function ProducerDepartments() {
  const { data: departments, isLoading } = useQuery({
    queryKey: ['producer-departments'],
    queryFn: async () => {
  ...
  const { data: memberCounts } = useQuery({
    queryKey: ['producer-department-counts'],
    queryFn: async () => {
  ...
  return (
    <MainLayout>
      <div className="space-y-6">
        <div><h1 className="text-3xl font-bold">Departments</h1><p className="text-muted-foreground">Overview of all departments ({departments?.length || 0} total)</p></div>
        {departments && departments.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {departments.map((dept) => (
              <Card key={dept.id}>
                <CardHeader className="pb-2"><CardTitle className="flex items-center gap-2"><Building2 className="h-5 w-5" style={{ color: dept.color || undefined }} />{dept.name}</CardTitle></CardHeader>
                <CardContent>
